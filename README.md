...
### ❓ 問題一：開啟 Web UI 後，狀態圓點是紅色，顯示連線失敗
...
3. 把 iPhone USB 線從 Mac 拔掉
4. 等待 5 秒
5. 重新插上 USB 線
6. iPhone 出現「信任這台電腦嗎？」→ 點**信任**
7. 重新開啟終端機 1，執行 `sudo python3 -m pymobiledevice3 remote start-tunnel`
8. 等待新的 RSD Address 和 Port 出現，抄下來
9. 重新開啟終端機 2，用新的數字執行

---

### ❓ 問題二：終端機 2 出現「Address already in use」
...
### ❓ 問題四：搖桿或地圖點擊了，但 iPhone 的定位沒有動

**檢查 1：狀態圓點是綠色嗎？**
- 不是綠色的話，先解決連線問題（見上方問題一）
...
### ❓ 問題五：iPhone GPS 停在假位置，想恢復真實 GPS
...
### ❓ 問題六：終端機顯示「No module named pymobiledevice3」
...
### ❓ 問題七：終端機顯示「command not found: python3」
...
| "Address already in use" error | Run: `lsof -ti:8765 \| xargs kill -9` |
...
*If you get stuck on any step or encounter an issue not listed here, feel free to open a GitHub Issue!*
